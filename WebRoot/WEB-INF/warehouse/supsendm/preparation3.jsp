							...
							<c:choose>
								...
								<c:otherwise>
									<c:if test="${isOnlyOne==1}">
										<c:if test="${p.isExtraSup==1}">
											<input type="button" class="frm_btn" id="supsendm_add" value="补发"
											onClick="window.self.location='<%=basePath%>warehouse/supsendm3/supsendm3_addExtraInput?preparation.id=${preparation.id }&needNum=${p.preNum }&itemType.id=${p.itemType.id}&isExtra=1'">
										</c:if>
										<c:if test="${p.isExtraSup!=1}">
											<input type="button" class="frm_btn" id="supsendm_add" value="制定"
											onClick="window.self.location='<%=basePath%>warehouse/supsendm3/supsendm3_addInput?preparation.id=${preparation.id }&needNum=${p.preNum }&itemType.id=${p.itemType.id}'">
										</c:if>
									</c:if>
									<c:if test="${isOnlyOne!=1}">
										<c:if test="${p.isExtraSup==1}">
											<input type="button" class="frm_btn" id="supsendm_add" value="补发"
											onClick="openWin('<%=basePath%>warehouse/supsendm3/supsendm3_addExtraInput?preparation.id=${preparation.id }&needNum=${p.preNum }&itemType.id=${p.itemType.id}&isExtra=1','editItemtype',800,600);return false">
										</c:if>
										<c:if test="${p.isExtraSup!=1}">
											<input type="button" class="frm_btn" id="supsendm_add" value="制定"
											onClick="openWin('<%=basePath%>warehouse/supsendm3/supsendm3_addInput?preparation.id=${preparation.id }&needNum=${p.preNum }&itemType.id=${p.itemType.id}','editItemtype',800,600);return false">
										</c:if>
									</c:if>
								</c:otherwise>
							</c:choose>
