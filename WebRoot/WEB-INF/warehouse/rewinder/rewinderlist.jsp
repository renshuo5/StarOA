			...
			<TR>
				<TD style="text-align: right">
					...
					<form action="warehouse/rewinder/rewinder_search" method="post">
						...
						<input type="button" name="SYS_SET" align="center" value="待入库"
							class="frm_btn" size=5
							onclick="javascript:self.location='<%=basePath%>warehouse/rewinder/rewinder_warehouseList'" />
						<input type="button" name="SYS_SET" value="添加 "
							class="frm_btn" size=5
							onclick="openWin('<%=basePath%>warehouse/rewinder/rewinder_addInput2','addRewinder',800,600);return false" />
					</form>
			...
			<c:if test="${!empty pm.datas}">
				<c:forEach items="${pm.datas}" var="rewinder">
					<tr>
						<td>
							<a
								href="warehouse/rewinder/rewinder_showQualify?RewinderId=${rewinder.rew_id}">
						...
						<td>
							${rewinder.warehouseDate}
						</td>
						<td class="examine">
					
						<c:if test="${null == rewinder.rewinspector && null == rewinder.whmanager}">
							<input name="status" type="text" value="${000}" />
						</c:if>
						<c:if test="${null != rewinder.rewinspector && null == rewinder.whmanager}">
						...
						<a href="#" onclick="openWin('<%=basePath%>warehouse/rewinder/rewinder_examineInput?rewinderId=${rewinder.rew_id}','examineRewinder',800,600,'1');return false">
						...
						<c:if test="${null == rewinder.whmanager}">
						<td>
