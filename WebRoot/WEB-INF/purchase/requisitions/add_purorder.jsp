...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<form action="purchase/purorder/purorder_add" method="post">
			...
			<table class="addTable" style="width: 90%">
				...
				<tr>
					...
					<td>
						<input type="text" name="purorder.pno" class="checkEmptyShowInfo"/>
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<input type="text" id="quaprolistnameId" class="selectin" disabled="disabled" 
							size="10">
						<input type="hidden" name="quaprolistId" id="quaprolistIdId">
						<input id="supplierinfoe" type="button" value="选择" class="frm_btn "
							onclick="openWin('<%=basePath%>purchase/quaprolist/quaprolist_selectInput?count=${pm.total}','selectQuaprolist',800, 600, 1); get()" />
					...
					<td>
						<input type="text" name="purorder.firm" class="checkEmptyShowInfo">
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<input type="text" disabled="disabled" id="businessnameId">
					</td>
					<td>
						订购日期：
					</td>
					<td>
						...
						<input type="hidden" id="d244_2" name="purorder.purdate" />
					</td>
				</tr>
			</table>
			<table class="list editTable" style="width:90%">
				<tr>
					<th>
						材料编码
					</th>
				...
				<c:if test="${!empty pm.datas}">
					<c:forEach items="${pm.datas}" var="shogood" varStatus="sr">
						...
						<c:set var="mul" value="${shogood.num*shogood.uprice}"></c:set>
						<c:set var="sum" value="${sum+mul}"></c:set>
						<tr align="center">
							<td>
								<input value="${shogood.itemType.mCode}" 
									disabled="disabled" size="8"/>
								<input type="hidden" value="${shogood.itemType.id}"
									name="itemtypeids" />
							</td>
							<td>
								<input value="${shogood.itemType.item.name }" 
									disabled="disabled" size="8" />
							</td>
							<td>
								<input value="${shogood.itemType.name}" 
									disabled="disabled" size="8"/>
							</td>
							<td>
								<input value="${shogood.unit}" disabled="disabled" size="6" />
								<input type="hidden" name="units" value="${shogood.unit}"  />
							</td>
							<td>
								<input value="${shogood.num}" id="num_${sr.index}"
									onBlur="add()"  name="nums" size="6"/>
							</td>
							<td>
								<input value="${shogood.uprice }" id="uprice_${sr.index}"
									disabled="disabled" size="8"/>
								<input type="hidden" name="uprices" value="${shogood.uprice }"
									 />
							</td>
							<td>
								<input id="d244_${sr.index }" type="text" class="Wdate checkDateInfo" size="8"
									onFocus="WdatePicker({dateFmt:'yyyy年M月d日',vel:'d2441_2_${sr.index }',skin:'whyGreen'})" />
								<input type="hidden" id="d2441_2_${sr.index}" name="delydates" />
							</td>
							<td>
								<input type="text" disabled="disabled" id="brandId_${sr.index}" size="8"/>
							</td>
							<td>
								<input name="notes" size="8"/>
							</td>
						</tr>
					</c:forEach>
				</c:if>
			...
			<table class="addTable" style="width: 90%">
				...
				<tr>
					...
					<td>
						<textarea name="purorder.pnote" cols="80" rows="6"></textarea>
					</td>
				</tr>
