...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<form action="" method="post">
				...
				<c:if test="${!empty resgoods}">
					<c:forEach items="${resgoods}" var="resgood" varStatus="sr">
						<tr align="center">
							...
							<td>
								<input name="unums" value="${resgood.unum}" size="7"
									disabled='disabled' />
							</td>
							<td>
          ...
          <tr>
               <td>
                   <input type='button' value='进料验收单' class='frm_btn' onClick="javascript:self.location='<%=basePath%>warehouse/feedexamine/feedexamine_addInput?requisitionsId=${requisitions.id}'"/>

               </td>
               <td>
                   <input type='button' value='收料通知单' class='frm_btn' onClick="javascript:self.location='<%=basePath%>warehouse/rewinder/rewinder_addInput?requisitionsId=${requisitions.id}'"/>
