			...
			<c:if test="${!empty pm.datas}">
				<c:forEach items="${pm.datas }" var="returned">
					<tr>
						...
						<td class="examine">
						 <c:if test="${returned.deals==null}">
							<c:if test="${ null== returned.signatory && null== returned.qualitymanager}">
						    ...
						    <a
									href="sales/return/returned/returned_examineInput?returnedId=${returned.retid}">
									 仓库审</a>&rarr;
							<a
									href="sales/return/returned/returned_qualityInput?returnedId=${returned.retid}">
									品工部审</a>&rarr;
							<a
									>
									已检验</a>	
									...
									<c:if test="${returned.deals=='1'}">
									 ...
									 <c:if test="${returned.buhege=='0'}">
						        	
						        	返工
						        	
						        	</c:if>
						        	<c:if test="${returned.buhege=='1'}">
						        	<input type="button" value="不合格报告表"
						   	 onClick="openWin('<%=basePath%>sales/return/unqualified/unqualified_addInput?returnedId=${returned.retid}','addUnqualified',1000,600); return false"
							  class="frm_btn">
						        	</c:if>
						        	<c:if test="${returned.buhege=='2'}">
						        	
							        <a href="<%=basePath%>sales/return/unqualified/unqualified_recheckpgb?unqualifiedId=${returned.unqualify_id}">返工复检</a>
						        	</c:if>
						        	</c:if>
						        	<c:if test="${returned.buhege=='3'}">
						            返工合格
						        	</c:if>
						...
						<td>
							...
							<a href="#"
								onclick="openWin('<%=basePath%>sales/return/returned/returned_editInput?unqualifiedId=${unqualified.unq_id}','editReturned',800,600);return false">
