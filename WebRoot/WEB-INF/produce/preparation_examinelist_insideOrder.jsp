			...
			<c:if test="${!empty pm.datas}">
				...
				<c:forEach items="${pm.datas }" var="preparation" varStatus="vs">
					...
					<tr>
						<td>
							<a href=""
								onclick="openWin('produce/preparation/preparation_showOutOrder?preparation.id=${preparation.id}','要货单信息',800,600,'1');return false"><c:out
						...
						<td class="examine">
							...
							<input name="status" id="as" type="hidden"
								value="${status[vs.index] }" />


							<!--<a style="display=none"
								href="produce/preparation/preparation_examineInputInside?preparation.id=${preparation.id}"><span>领导审</span>
							</a>
							--><a
								href="produce/preparation/preparation_pingongExamineInputInside?preparation.id=${preparation.id}"><span>品工部审</span>
							</a>&rarr;
							<a
								href="produce/preparation/preparation_produceExamineInputInside?preparation.id=${preparation.id}"><span>生产部审</span>
							</a>
