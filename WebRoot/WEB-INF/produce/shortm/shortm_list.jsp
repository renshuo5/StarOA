...
<form action="produce/shortm/shortm_search" method="post">
            ...
            申请人:
               <select name="shortm.employee2.empName" >
              <option>请选择</option>
        ...
        状态： <input type="text" name="shortm.state" class="itemname">
            ...
            <input type="submit" value="查询" class="frm_btn">
             <input type="button" onclick="openWin('<%=basePath %>produce/shortm/shortm_register_add',600,800,600);return false" value="添加" class="frm_btn"></td></tr>
 </table>
                </form>
        <table class="title">
			<tr>
  	...
  	<table  class="list">
		...
		<td align="center">${shortm.department1.dptName }</td>
					<td align="center">${shortm.employee2.empName}</td>
                             ...
                             <td class="examine">
                             <c:if test="${shortm.state==0}">
                               <c:if test="${shortm.sho_check==null}">
                               <input type="hidden" name="status" value="00"/>
                               </c:if>
								<c:if test="${shortm.sho_check!=null}">
								<input type="hidden" name="status" value="10"/>
								</c:if>
									<a href="#" onclick="openWin('<%=basePath%>produce/shortm/shortm_shortm_approve?step=1&shortmID=${shortm.id}',600,800,600);return false" ><span>审核人</span>
								</a>&rarr;
			
								<a href="#" onclick="openWin('<%=basePath%>produce/shortm/shortm_shortm_approve?step=2&shortmID=${shortm.id}',600,800,600);return false" ><span>批准人</span>
								</a>
								 </c:if>
				 ...
				 <a href="<%=basePath%>produce/shortm/shortm_shortm_approve?shortmID=${shortm.id}">
                  ...
                  已审核
                                ...
                                <c:if test="${shortm.state==0}">
                                   ...
                                   <a class="a03" href="#"
                             onclick="openWin('<%=basePath %>produce/shortm/shortm_edit_input?shortmID=${shortm.id}','editshortm',800,600);return false"
                            ><img src="images/edit.gif" width="15" height="16" border="0"></a>
                           </c:if>
                               
							</td>
                                  <td align="center"><a href="#"
								onclick="del('<%=basePath %>produce/shortm/shortm_shortm_del?shortmID=${shortm.id}');return false">
